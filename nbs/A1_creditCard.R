...
# Descripcion dataset:
#    Límite tarjeta (NT dollar: individual consumer credit and his/her family, supplementary, credit)
...
#        Valores:  -1 = pagado; 1 = retraso 1 mes; 2 = 2 meses; . . .; 8 = 8 meses; 9 = 9 meses o más
#    Importe liquidación: bill_amt1 = Sept2005 a bill_amt6=  Abril 2005)
#    pay_amt1-pay_amt6 (importe pagos realizados liquidaciones anteriores
...
# Previo al matching 
# Análisis de potenciales variable numéricas que
# pueden influir en el estatus de tratamiento (impagoAbril)
...
m.exact.data[m.exact.data$subclass==6,] # Saldo abril = $290 NT
m.exact.data[m.exact.data$subclass==1,] # INCLUYE TODAS las liquidaciones con saldo abril=0
...
glimpse(m.nn.4.data) 
# Interpretación de los pesos: una unidad de control se empareja con una unidad tratada que ya tiene dos
# unidades de control emparejadas a ella; esa misma unidad de control se emparejada con una unidad tratada 
# que tiene otra unidad de control emparejada a ella. La unidad de control en cuestión
# recibiría un peso de 1/3 + 1/2 = 5/6.

# NO OBSTANTE, si replace=TRUE es prefeirble generar DF emparejado llamando a la función get_matches (en lugar de match_data)
...
m.nn.4.data <- get_matches(m.nn.4,id = "idx")
glimpse(m.nn.4.data) 
# Veamos las primeras 30 unidades de control
m.nn.4.data[m.nn.4.data$impagoAbril==FALSE,"id"] %>% sort %>% head(30)
# Vemos que id=1 ha sido utilizada en múltiples matches!
...
# "Además de simplemente lograr el equilibrio, es importante informar sobre el equilibrio
# para convencer al lector de que el análisis se realizó adecuadamente y que las
# conclusiones causales son válidas" (Thoemmes & Kim, 2011). El equilibrio de covariables es
# típicamente evaluado a partir de medidas estadísticas, incluyendo
# diferencias de medias estandarizadas, razones de varianza y valores p de pruebas t o KS.
# El equilibrio puede ser reportado en un artículo por medio de tablas de equilibrio o gráficas
# que muestran las medidas de equilibrio antes y después del matching.
# Si se usa y presenta una medida de equilibrio defendible, se permite que el lector
# juzgue la validez de la afirmación causal, basándose en los métodos utilizados 
...
# Para las diferencias de medias estandarizadas, se han propuesto umbrales de .1 y .25,
# pero Stuart et al. (2013) encuentran que el primero (.1) es más efectivo
...
# Evaluar el equilibrio de las covariables del resto de muestras
# ¿Qué muestra elegiríamos?
...
love.plot(m.nn.1,stats = c("m","var","ks"),thresholds=c(m=0.1,ks=0.05,var=2))
...
# Exact matching
m.exact.m <- matchit(impagoAbril~importeAbril+age+education+limit_bal,
                  method="exact",
                  data=impago) 
m.exact.m
...
# Continuar con otros de métodos de emparejamiento. Decidir cual 
